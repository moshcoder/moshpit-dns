 ...
 */
export function installPlan({ platform, node, entry, port, ttl, registryBase, home = homedir() }) {
  ...
  if (platform === "macos") {
    const path = posix.join(home, "Library", "LaunchAgents", `${SERVICE_LABEL}.plist`);
  ...
  if (platform === "linux") {
    const path = posix.join(home, ".config", "systemd", "user", `${SERVICE_NAME}.service`);
    return {
      platform,
      elevated: false,
      steps: [
